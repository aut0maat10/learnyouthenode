...
var server = http.createServer(function (request, response) {  // callback takes request + response as arguments
    
    response.writeHead(200, { 'content-type': 'text/plain' }) // Sends a response header to the request.
  
          
  
  fs.createReadStream(process.argv[3]).pipe(response)   // creates a stream representing  
 })                             // the file you are given as a command-line argument  
